 ...
 */
  ...
  */
function start() {
  alert("Þetta er leikur sem gengur út á að giska á tölu á bilinu 1-100")
  //while (confirm("Viltu spila annan leik?")) { play(); }
  do {
    play();
    if (breaker == true) break;
  } while(confirm("Viltu spila annan leik?")) 
  let nidurstodur = getResults();
  alert(nidurstodur);
}
...
function play() {
  ...
  let count = 0;
  //let agiskun; //profa
  //if (agiskun === null) {return;} //profa
  while (gisk != random) {
    let agiskun = prompt("Ágiskun:");
    if (agiskun === null) {
      alert('Hætt í leik');
      breaker = true;
      break;
    }
    gisk = parseGuess(agiskun);
    let svar = getResponse(gisk, random);
    alert(svar);
    count++;
  }
  if (gisk == random) {
...
function parseGuess(input){
  ...
  let gisktemp = parseInt(input, 10);
  return gisktemp;
  }
}
